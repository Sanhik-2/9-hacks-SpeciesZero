...
def validate_update_state(data):
    ...
    if not isinstance(data, dict):
        return False, "Payload must be a JSON object"
    required = ["phenomenon_id", "previous_phenomenon_id", "damage_taken", "damage_to_player", "user_hp", "ai_hp", "is_player_dead", "state", "action", "next_state"]
    for req in required:
        if req not in data:
            return False, f"Missing required field: {req}"
    return True, None
